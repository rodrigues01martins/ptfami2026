...
const initialState = {
  ...
  ocorrencias: {
    linhas: [{ data: "", ocorrencia: "", impacto: "", providencia: "", situacao: "" }]
  },
  financeiro: {
  ...
  dificuldades: {
    linhas: [{ meta: "", desvio: "", causa: "", medida: "", resultado: "" }]
  },
  conclusao: {
    status: "integralmente cumprido", qtdAtingida: "", qtdParcial: "", qtdNao: "", continuidade: ""
  },
  declaracao: {
    municipio: "", data: "", nome: "", cargo: "Fundação de Apoio ao Menor Inhumense – FAMI"
  },
...
const Field = ({ label, value, onChange, type = 'text', hint = '', textarea = false, locked = false }: {
  ...
  <div className="mb-4">
    <label className="block text-xs font-bold text-slate-600 mb-1 uppercase tracking-wide">
      ...
      {locked && <span className="ml-2 px-2 py-0.5 bg-[#e3ede4] text-[#3c6b4f] text-[10px] font-bold rounded-full uppercase tracking-wide">fixo</span>}
    </label>
    {textarea ? (
      <textarea
...
const SectionHead = ({ n, title, sub }: { n: number; title: string; sub: string }) => (
  <div className="mb-6">
    <div className="text-xs font-mono text-[#b8912f] tracking-widest uppercase mb-1">Seção {n} de {STEPS.length}</div>
    <h1 className="text-2xl font-bold text-[#1c2d45] mb-2">{title}</h1>
...
const Card = ({ title, children }: { title?: string; children: React.ReactNode }) => (
  <div className="bg-white border border-slate-200 rounded-xl p-6 mb-4 shadow-sm">
    {title && <h3 className="text-sm font-bold text-[#1c2d45] border-b border-slate-100 pb-3 mb-4">{title}</h3>}
    {children}
  </div>
);
...
const SecExecucao = ({ s, set }: { s: AppState; set: (fn: (p: AppState) => AppState) => void }) => (
  <>
    <SectionHead n={3} title="Execução do objeto"
      sub="Implantação do CASER Rio Verde e manutenção da gestão socioeducativa." />
        ...
        <table className="w-full text-sm border-collapse mt-2">
          <thead>
            <tr className="bg-slate-50">
              {['Entrega','Prazo','Situação','Evidência','Observação'].map(h => (
                <th key={h} className="text-left text-xs font-bold text-slate-500 uppercase p-2 border-b border-slate-200">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {s.execucao.entregas.map((e, i) => (
              <tr key={i} className="border-b border-slate-100">
                <td className="p-2 text-slate-700 text-sm">{e.nome}</td>
                <td className="p-2 text-slate-500 text-sm whitespace-nowrap">{e.prazo}</td>
                <td className="p-2">
                  <select className="w-full border border-slate-200 rounded-lg p-2 text-xs focus:ring-2 focus:ring-[#00735C] outline-none"
                    value={e.situacao}
                ...
                <td className="p-2"><input className="w-full border border-slate-200 rounded-lg p-2 text-xs focus:ring-2 focus:ring-[#00735C] outline-none" value={e.evidencia} placeholder="Anexo"
                ...
                <td className="p-2"><input className="w-full border border-slate-200 rounded-lg p-2 text-xs focus:ring-2 focus:ring-[#00735C] outline-none" value={e.obs}
...
const SecPublico = ({ s, set }: { s: AppState; set: (fn: (p: AppState) => AppState) => void }) => (
  <>
    <SectionHead n={4} title="Público atendido"
      sub="Movimentação mensal de adolescentes atendidos no período do relatório." />
    <Card>
      <div className="overflow-x-auto">
        <table className="w-full text-sm border-collapse">
          <thead>
            <tr className="bg-slate-50">
              <th className="text-left text-xs font-bold text-slate-500 uppercase p-2 border-b border-slate-200">Informação</th>
              {s.publico.meses.map(m => <th key={m} className="text-left text-xs font-bold text-slate-500 uppercase p-2 border-b border-slate-200 whitespace-nowrap">{m}</th>)}
              <th className="text-left text-xs font-bold text-slate-500 uppercase p-2 border-b border-slate-200">Total/média</th>
            </tr>
          </thead>
          <tbody>
...
const SecResultados = ({ s, set }: { s: AppState; set: (fn: (p: AppState) => AppState) => void }) => {
  const rows: { key: keyof typeof s.resultados; label: string }[] = [
    { key: 'atingida', label: 'Atingida' },
    { key: 'parcial', label: 'Parcialmente atingida' },
    { key: 'naoAtingida', label: 'Não atingida' },
    { key: 'naoAplicavel', label: 'Não aplicável' },
  ];
  return (
    <>
      ...
      <Card>
        <div className="overflow-x-auto">
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="bg-slate-50">
                {['Situação','Quantidade','Indicadores','Justificativa/providência'].map(h => (
                  <th key={h} className="text-left text-xs font-bold text-slate-500 uppercase p-2 border-b border-slate-200">{h}</th>
                ))}
              </tr>
            </thead>
                  ...
                  <tr key={key} className="border-b border-slate-100">
                    ...
                    {(['qtd','indic','just'] as const).map(f => (
                      <td key={f} className="p-1">
                        <input className="w-full border border-slate-200 rounded p-1.5 text-xs focus:ring-2 focus:ring-[#00735C] outline-none" value={row[f]}
...
const SecEquipe = ({ s, set }: { s: AppState; set: (fn: (p: AppState) => AppState) => void }) => (
  <>
    <SectionHead n={6} title="Equipe de trabalho"
      sub="Efetivo previsto e situação real da equipe no período." />
    <Card>
      <div className="overflow-x-auto">
        <table className="w-full text-sm border-collapse">
          <thead>
            <tr className="bg-slate-50">
              {['Categoria','Previsto','Média em exercício','Vacâncias/afastamentos','Medida adotada'].map(h => (
                <th key={h} className="text-left text-xs font-bold text-slate-500 uppercase p-2 border-b border-slate-200">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {s.equipe.linhas.map((l, i) => (
              <tr key={i} className="border-b border-slate-100">
                <td className="p-2 text-slate-700 text-sm">{l.cat}</td>
                <td className="p-2 text-slate-500 text-sm">{l.previsto}</td>
                {(['media','vac','medida'] as const).map(f => (
                  <td key={f} className="p-1">
                    <input className="w-full border border-slate-200 rounded p-1.5 text-xs focus:ring-2 focus:ring-[#00735C] outline-none" value={l[f]}
  ...
  <div>
    <div className="overflow-x-auto">
      <table className="w-full text-sm border-collapse">
        <thead>
          <tr className="bg-slate-50">
            {cols.map(c => <th key={c.key} className="text-left text-xs font-bold text-slate-500 uppercase p-2 border-b border-slate-200">{c.label}</th>)}
            ...
            <tr key={i} className="border-b border-slate-100">
              {cols.map(c => (
                <td key={c.key} className="p-1">
                  <input className="w-full border border-slate-200 rounded p-1.5 text-xs focus:ring-2 focus:ring-[#00735C] outline-none" value={row[c.key]}
                    onChange={ev => onEdit(i, c.key, ev.target.value)} />
                </td>
              ))}
    ...
    <button onClick={onAdd} className="mt-3 px-4 py-2 bg-[#f1e6c8] border border-[#b8912f] text-[#7a5c00] text-xs font-bold rounded-lg hover:bg-[#e9dcae] transition-colors">
      + Adicionar linha
    </button>
  </div>
);
    ...
    <Card title="Execução por grupo de despesa">
      <div className="overflow-x-auto">
        <table className="w-full text-sm border-collapse">
          <thead>
            <tr className="bg-slate-50">
              {['Grupo de despesa','Previsto (R$)','Executado (R$)','%','Variação justificada'].map(h => (
                <th key={h} className="text-left text-xs font-bold text-slate-500 uppercase p-2 border-b border-slate-200">{h}</th>
              ))}
            </tr>
          </thead>
            ...
            {s.financeiro.despesas.map((d, i) => (
              <tr key={i} className="border-b border-slate-100">
                <td className="p-2 text-slate-700 text-sm">
                  {d.grupo}
                  <span className="ml-2 px-1.5 py-0.5 bg-[#e3ede4] text-[#3c6b4f] text-[10px] font-bold rounded-full">fixo</span>
                </td>
                <td className="p-2 text-slate-500 text-sm">{d.previsto}</td>
                {(['executado','pct','variacao'] as const).map(f => (
                  <td key={f} className="p-1">
                    <input className="w-full border border-slate-200 rounded p-1.5 text-xs focus:ring-2 focus:ring-[#00735C] outline-none" value={d[f]}
    ...
    <Card>
      <div className="mb-4">
        ...
        <select className="w-full border border-slate-200 rounded-lg p-3 text-sm focus:ring-2 focus:ring-[#00735C] outline-none bg-white"
          value={s.conclusao.status}
          onChange={e => set(p => ({ ...p, conclusao: { ...p.conclusao, status: e.target.value } }))}>
...
const SecAnexos = ({ s, set, onPreview }: { s: AppState; set: (fn: (p: AppState) => AppState) => void; onPreview: () => void }) => (
  <>
    <SectionHead n={12} title="Índice mínimo de anexos"
      sub="Marque os anexos que acompanham este relatório." />
    <Card>
      {ANEXOS_LIST.map((a, i) => (
        <div key={i} className="flex items-start gap-3 mb-3">
          ...
          <span className="text-sm text-slate-700">
            <strong className="text-slate-500">Anexo {romanize(i + 1)}</strong> — {a}
          </span>
        </div>
      ))}
    </Card>
...
const Preview = ({ s, onBack }: { s: AppState; onBack: () => void }) => {
  ...
  const tableStyle = { width: '100%', borderCollapse: 'collapse' as const, marginBottom: 12, fontSize: 12 };
  const thStyle = { border: '1px solid #999', padding: '5px 7px', background: '#eee', textAlign: 'left' as const };
  const tdStyle = { border: '1px solid #999', padding: '5px 7px', textAlign: 'left' as const, verticalAlign: 'top' as const };
  const h2Style = { fontSize: 14, textTransform: 'uppercase' as const, borderBottom: '1.5px solid #1a1a1a', paddingBottom: 4, margin: '24px 0 10px' };
  ...
  return (
    <div>
      ...
      <div className="flex gap-3 justify-between items-center mb-6 sticky top-0 bg-[#f8fafc] py-3 z-10 border-b border-slate-200">
        ...
        <button onClick={() => window.print()} className="flex items-center gap-2 px-5 py-2 bg-[#b8912f] text-white font-bold rounded-xl hover:bg-[#a67f28] transition-all text-sm">
      ...
      <div id="report-page" style={{ background: '#fff', border: '1px solid #ddd', padding: '52px 56px', fontFamily: '"Times New Roman", serif', color: '#1a1a1a', fontSize: 13, lineHeight: 1.55, maxWidth: 820, margin: '0 auto' }}>
        ...
        <h2 style={h2Style}>1. Identificação da parceria</h2>
        ...
        <h2 style={h2Style}>2. Resumo executivo</h2>
        <p style={{ whiteSpace: 'pre-wrap' }}>{esc(s.resumo.texto)}</p>

        <h2 style={h2Style}>3. Execução do objeto</h2>
        <p><em>3.1 Implantação do CASER Rio Verde</em></p>
        <p style={{ whiteSpace: 'pre-wrap' }}>{esc(s.execucao.implantacao)}</p>
        <table style={tableStyle}>
          <thead><tr>{['Entrega','Prazo','Situação','Evidência','Observação'].map(h => <th key={h} style={thStyle}>{h}</th>)}</tr></thead>
          <tbody>{s.execucao.entregas.map((e,i) => <tr key={i}>{[e.nome,e.prazo,e.situacao||'—',e.evidencia||'—',e.obs||'—'].map((v,j) => <td key={j} style={tdStyle}>{v}</td>)}</tr>)}</tbody>
        </table>
        <p><em>3.2 Manutenção da gestão socioeducativa</em></p>
        <p style={{ whiteSpace: 'pre-wrap' }}>{esc(s.execucao.manutencao)}</p>

        <h2 style={h2Style}>4. Público atendido</h2>
        <table style={tableStyle}>
          <thead><tr><th style={thStyle}>Informação</th>{s.publico.meses.map(m => <th key={m} style={thStyle}>{m}</th>)}<th style={thStyle}>Total/média</th></tr></thead>
          <tbody>{s.publico.linhas.map((l,i) => <tr key={i}><td style={tdStyle}>{l.label}</td>{l.valores.map((v,j) => <td key={j} style={tdStyle}>{v||'—'}</td>)}<td style={tdStyle}>{l.total||'—'}</td></tr>)}</tbody>
        </table>
        <p><strong>Análise:</strong> {esc(s.publico.analise)}</p>

        <h2 style={h2Style}>5. Resultados e alcance das metas</h2>
        <table style={tableStyle}>
          <thead><tr>{['Situação','Quantidade','Indicadores','Justificativa/providência'].map(h => <th key={h} style={thStyle}>{h}</th>)}</tr></thead>
          <tbody>
            {[['Atingida', s.resultados.atingida],['Parcialmente atingida', s.resultados.parcial],['Não atingida', s.resultados.naoAtingida],['Não aplicável', s.resultados.naoAplicavel]].map(([label, r]: any) => (
              <tr key={label}><td style={tdStyle}>{label}</td><td style={tdStyle}>{r.qtd||'—'}</td><td style={tdStyle}>{r.indic||'—'}</td><td style={tdStyle}>{r.just||'—'}</td></tr>
            ))}
          </tbody>
        </table>
        ...
        <h2 style={h2Style}>6. Equipe de trabalho</h2>
        <table style={tableStyle}>
          <thead><tr>{['Categoria','Previsto','Média em exercício','Vacâncias/afastamentos','Medida adotada'].map(h => <th key={h} style={thStyle}>{h}</th>)}</tr></thead>
          <tbody>{s.equipe.linhas.map((l,i) => <tr key={i}><td style={tdStyle}>{l.cat}</td><td style={tdStyle}>{l.previsto}</td><td style={tdStyle}>{l.media||'—'}</td><td style={tdStyle}>{l.vac||'—'}</td><td style={tdStyle}>{l.medida||'—'}</td></tr>)}</tbody>
        </table>
        <p><strong>Formação continuada:</strong> {esc(s.equipe.formacao)}</p>

        <h2 style={h2Style}>7. Ocorrências relevantes, riscos e providências</h2>
        <table style={tableStyle}>
          <thead><tr>{['Data/período','Ocorrência/risco','Impacto','Providência','Situação atual'].map(h => <th key={h} style={thStyle}>{h}</th>)}</tr></thead>
          <tbody>{s.ocorrencias.linhas.map((r,i) => <tr key={i}><td style={tdStyle}>{r.data||'—'}</td><td style={tdStyle}>{r.ocorrencia||'—'}</td><td style={tdStyle}>{r.impacto||'—'}</td><td style={tdStyle}>{r.providencia||'—'}</td><td style={tdStyle}>{r.situacao||'—'}</td></tr>)}</tbody>
        </table>

        <h2 style={h2Style}>8. Execução financeira sintética</h2>
        <table style={tableStyle}>
          <thead><tr><th style={thStyle}>Informação</th><th style={thStyle}>Valor (R$)</th></tr></thead>
          <tbody>
            {[['Valor global pactuado', FIXED.valorGlobal],['Valor efetivamente transferido', s.financeiro.transferido],['Rendimentos de aplicação', s.financeiro.rendimentos],['Valor utilizado no objeto', s.financeiro.utilizado],['Saldo bancário final', s.financeiro.saldo],['Valor devolvido', s.financeiro.devolvido],['Reserva/provisão verbas rescisórias', s.financeiro.reserva]].map(([k,v]) => (
              <tr key={k}><td style={tdStyle}>{k}</td><td style={tdStyle}>{v||'—'}</td></tr>
            ))}
          </tbody>
        </table>
        <table style={tableStyle}>
          <thead><tr>{['Grupo de despesa','Previsto (R$)','Executado (R$)','%','Variação justificada'].map(h => <th key={h} style={thStyle}>{h}</th>)}</tr></thead>
          <tbody>{s.financeiro.despesas.map((d,i) => <tr key={i}><td style={tdStyle}>{d.grupo}</td><td style={tdStyle}>{d.previsto}</td><td style={tdStyle}>{d.executado||'—'}</td><td style={tdStyle}>{d.pct||'—'}</td><td style={tdStyle}>{d.variacao||'—'}</td></tr>)}</tbody>
        </table>
        <p><strong>Declaração sobre a aplicação dos recursos:</strong> {esc(s.financeiro.declaracao)}</p>

        <h2 style={h2Style}>9. Dificuldades, desvios e medidas corretivas</h2>
        <table style={tableStyle}>
          <thead><tr>{['Meta/atividade','Desvio identificado','Causa','Medida corretiva','Resultado/prazo'].map(h => <th key={h} style={thStyle}>{h}</th>)}</tr></thead>
          <tbody>{s.dificuldades.linhas.map((r,i) => <tr key={i}><td style={tdStyle}>{r.meta||'—'}</td><td style={tdStyle}>{r.desvio||'—'}</td><td style={tdStyle}>{r.causa||'—'}</td><td style={tdStyle}>{r.medida||'—'}</td><td style={tdStyle}>{r.resultado||'—'}</td></tr>)}</tbody>
        </table>

        <h2 style={h2Style}>10. Conclusão da OSC</h2>
        ...
        <h2 style={h2Style}>11. Declaração e assinatura</h2>
        ...
        <p>{esc(s.declaracao.municipio)}, {fmtDate(s.declaracao.data)}.</p>
        ...
        <h2 style={h2Style}>12. Índice mínimo de anexos</h2>
        <ul style={{ paddingLeft: 18 }}>
          {ANEXOS_LIST.map((a, i) => (
            <li key={i} style={{ marginBottom: 4 }}>{s.anexos.checks[i] ? '☑' : '☐'} Anexo {romanize(i+1)} — {a}</li>
          ))}
        </ul>
      </div>
    </div>
...
export default function RelatorioFinal({ onBack }: { onBack: () => void }) {
  ...
  const handleExport = () => {
    ...
    a.href = url; a.download = 'relatorio-final-progresso.json'; a.click();
  ...
  const handleImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      try { setState(s => ({ ...s, ...JSON.parse(reader.result as string) })); }
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] flex">
      ...
      {/* ── Sidebar ── */}
      <aside className="w-72 bg-[#1c2d45] text-slate-200 flex flex-col sticky top-0 h-screen overflow-y-auto flex-shrink-0">
        {/* Topo */}
        <div className="p-5 border-b border-white/10">
          ...
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full border border-[#b8912f] flex items-center justify-center text-[#b8912f] text-xs font-bold flex-shrink-0">TC</div>
            <div>
              <span className="text-[10px] uppercase tracking-widest text-slate-400 block">SEDS · Goiás</span>
              <span className="text-sm font-bold text-slate-100">Relatório Final</span>
            </div>
          </div>
        </div>
        ...
        <div className="px-5 py-4 border-b border-white/10">
          <div className="flex justify-between text-xs mb-2">
            <span className="text-slate-400 uppercase tracking-wider">Preenchimento</span>
            <span className="text-[#b8912f] font-bold">{progress}%</span>
          </div>
          <div className="h-1.5 bg-white/10 rounded-full overflow-hidden">
            <div className="h-full bg-gradient-to-r from-[#b8912f] to-[#d9b869] transition-all duration-500" style={{ width: `${progress}%` }} />
          </div>
        </div>
          ...
          {STEPS.map((s, i) => {
            ...
            return (
              <button key={s.id} onClick={() => setCurrentStep(i)}
                className={`w-full flex items-start gap-3 px-3 py-2.5 rounded-lg mb-1 text-left transition-all ${active ? 'bg-[#b8912f]/20 text-white' : 'hover:bg-white/5 text-slate-300'}`}>
                <span className={`w-5 h-5 rounded-full border flex items-center justify-center text-[10px] flex-shrink-0 mt-0.5 font-bold
                  ${done ? 'bg-[#3c6b4f] border-[#3c6b4f] text-white' : active ? 'border-[#b8912f] text-[#b8912f]' : 'border-slate-600 text-slate-500'}`}>
                  {done ? '✓' : i + 1}
                </span>
        ...
        <div className="p-4 border-t border-white/10 space-y-2">
          <button onClick={handleExport} className="w-full flex items-center gap-2 px-3 py-2 text-xs border border-white/20 rounded-lg text-slate-300 hover:border-[#b8912f] hover:text-white transition-all">
            <Download size={13} /> Salvar progresso (.json)
          </button>
          <label className="w-full flex items-center gap-2 px-3 py-2 text-xs border border-white/20 rounded-lg text-slate-300 hover:border-[#b8912f] hover:text-white transition-all cursor-pointer">
            <Upload size={13} /> Carregar progresso
      ...
      <main className="flex-1 p-8 max-w-3xl overflow-y-auto">
        ...
        {/* Navegação (exceto na seção de anexos que tem botão próprio) */}
